...
function User(props) {
    ...
    function handleChange(choice) {
        if (window.location.pathname === '/dashboard') {
            ...
            // For all other choices, go to home page and set the state
    ...
    return <>
        ...
        <ul id="user_list">
            {(user.account_type && decryptData(user.account_type) === 2) ? <li onClick={() => window.location.href = '/dashboard'}>
                <span><FontAwesomeIcon icon={faGear} />Admin Dashboard</span>
            </li> : null}
            {(page.game || page.game_page || window.location.pathname === '/dashboard') ? <li onClick={() => { handleChange('salon') }}>

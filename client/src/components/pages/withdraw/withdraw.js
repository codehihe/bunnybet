...
import { getData, isEmpty } from '../../../utils/utils'
...
function Withdraw(props){    
    ...
    const [allowedCurrencies, setAllowedCurrencies] = useState(['INR', 'USD', 'EUR', 'GBP', 'CHF', 'RON'])

    useEffect(() => {
        getData('/api/admin/config').then(data => {
            if(data && data.WITHDRAWAL_CURRENCIES){
                setAllowedCurrencies(data.WITHDRAWAL_CURRENCIES.split(',').map(s => s.trim()))
            }
        })
    }, [])
    ...
    const errors_default = {
        ...
        withdraw_method: { fill: true, fill_message: "fill_field", validate: true },
        withdraw_details: { fill: true, fill_message: "fill_field", validate: true },
    ...
    const [formState, setFormState] = useState({ 
        ...
        withdraw_details: '', // Holds UPI ID or Bank Details
    })
    ...
    function validateForm() {
        const { name, phone, email, country, city, withdraw_details } = formState        
        ...
        if (isEmpty(withdraw_details)) {
            errors.withdraw_details.fill = false
        }
                    ...
                    /> : <>
                        <Row>
                            <Col sm={12}>
                                ...
                                <p><b>{translate({lang, info: "convert_carrots_rate"})}: </b>{convert_carrots_rate} = 1 INR</p>
                            </Col>
                        </Row>                        

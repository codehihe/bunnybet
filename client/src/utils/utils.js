  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        console.warn(`[API Error] 404 Not Found: ${url}`);
        // Return a safe error object that won't crash standard checks
        return { error: true, status: 404, message: "Endpoint not found", result: "error", payload: "404_not_found" };
      }
    ...
    console.error(`[API Exception] POST ${url}`, error);
    throw error;
  } 
}
...
export const getData = async (url = "")=>{
  ...
  try {
    ...
    const result = await response.json();
    console.log(`[API Response] GET ${url}`, result);
    return result;
  } catch (error) {
    console.error(`[API Exception] GET ${url}`, error);
    throw error;
  } 
}
...
export const convertCurrency = (value=0, currency="INR", exchangeRates=null, shorten=true, base="INR")=>{  
  if (value <= 0) {
  ...
  if(!exchangeRates){
    ...
    // Fallback logic if exchange rates are missing
    if(base === "INR" && currency === "USD") return value / 85
    if(base === "USD" && currency === "INR") return value * 85
    console.error("convertCurrency-exchangeRates-error--> ", exchangeRates)
    return value
  }

  const baseRate = exchangeRates[base] || (base === "USD" ? 1.0 : 85)
  const targetRate = exchangeRates[currency]

  if(!targetRate){
    console.error("convertCurrency-rate-error--> ", targetRate, currency)
    if(currency === base) return value
    return value / baseRate 
  }

  // Conversion logic: rates are relative to USD (USD=1.0).
  // value_in_usd = value / baseRate
  // value_in_target = value_in_usd * targetRate
  let result = (parseFloat(value) / baseRate) * targetRate
